...
export const talentTierSchema = z.enum([
  ...
  'keystone',
  /**
   * キーストーンのさらに先。**ボード全体で 1 つしか取れない**（03-progression.md ⑧）。
   *
   * ポイントはいずれ余る（ランクの供給に上限が無いに等しい）ので、
   * 終盤の「どこに寄せるか」をポイントで作ることはできない。
   * **最後の 1 つだけは全体で排他**にして、そこに選択を残している。
   * どれも大きな見返りと引き換えに、はっきりした代償を持つ
   */
  'capstone',
]);
