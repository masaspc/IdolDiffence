 ...
 */
export function resolveUnit(unit: Unit, runPool: ModifierPool, specialActive: boolean): void {
  ...
  // ポジション強化・覚醒・スペシャルは乗算プールへ入れる（枠が有限なので暴走しにくい）
  ...
  unit.attack = resolveAttack(unit, branch ? unit.awakening : null, runPool);
}
...
function resolveAttack(
  ...
  // 減速の効果量はカードでも伸びる
  const slowPower = resolveStat(1, 'slowPower', [runPool]);
