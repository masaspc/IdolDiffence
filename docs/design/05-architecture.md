...
| 描画（`renderer.draw`） | 2.0〜5.0ms | 2.5〜4.2ms | 15〜30% |

描画の側は**走らせるたびに 2 倍ほどぶれる**（同じ seed・同じフレーム列でも）。
ブラウザの GC と JIT の都合なので、1 回の数字を丸で信じない ——
以下の比較も「桁が変わったか」で読む。
...
**全体 p99 は 9.2ms → 2.5〜4.2ms、200〜299 体では 11.3ms → 2.0〜5.0ms** になった。
最悪値は 77.4ms → 12〜13ms で、予算を超えるフレームは消えた。
