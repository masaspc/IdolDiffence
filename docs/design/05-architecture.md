...
計測ページは**本番と同じ演出配線**（`ui/battleEffects.ts`）を使い、
`advanceEffects` と HUD の帯（`setSafeArea`）も本番と同じに呼ぶ。
最初に書いたときはこれを省いていて、カットインも閃光もコメントもリングも
出ない絵を測っていた —— **本番より軽い数字を本番の数字として出していた**
（Codex の指摘で気づいた）。

| 層 | 敵 200〜299 体 p99 | 全体 p99 | 予算 16.67ms に対して |
|---|---|---|---|
| sim（`world.update`）+ `snapshot` | 0.53ms | 0.06ms | 3% |
| 描画（`renderer.draw`） | 1.9〜2.1ms | 2.3〜3.1ms | 12〜19% |

**重いのは描画で、sim ではない。** 差は 1〜2 桁ある。5.11 のリスク表が
「Canvas 2D の描画限界」を挙げていたのは当たっていて、詰めるならこちら側になる。

#### 直したところ

山は**敵ごとの `createRadialGradient`**（足元に敷く属性の光）だった。
1 体なら誤差だが、200 体 × 60 回/秒になるとまとまった時間になる。
色と丸めた半径で焼いて使い回すようにした（`renderer.ts` の `enemyGlow`）。

**1 回の計測を信じない。** 同じ seed・同じフレーム列でも走らせるたびにぶれるので、
前後それぞれ 3 回ずつ回して並べる（S30 ★10 ×6、dpr 1）。

| | before（3 回） | after（3 回） |
|---|---|---|
| 全体 p99 | 3.7 / 3.5 / 3.7ms | **3.1 / 2.3 / 2.8ms** |
| 敵 300 体以上 p50 | 2.4 / 2.4 / 2.3ms | **1.7 / 1.7 / 1.8ms** |
| 敵 200〜299 体 p99 | 3.1 / 2.2 / 2.5ms | **1.9 / 2.1 / 2.0ms** |
| 最悪値 | 32.1 / 34.1 / 32.4ms | 32.1 / 32.2 / 46.6ms |

効きは**ばらつきを超える程度で、劇的ではない**。敵が多いほど差が開き、
300 体以上の p50 では 2.4ms → 1.7ms と 2 群が重ならない。
一方**最悪値は変わらない** —— そこは敵の数の話ではないから（下記）。

同じフレームを前後で撮って**ピクセル単位で一致**することも確認してある
（1280×480 の 614400 px すべてで差 0）。見た目は 1 ドットも変えていない。
焼いた表が太らないことは `src/render/glow.test.ts` が見張る。

#### 残っている山：開幕の 1 フレーム

**予算を超えるフレームは 1693 中 1 つあり、それは敵が多い場面ではなく開幕**。
32〜47ms 掛かる。背景と盤面の静的レイヤを焼き、ドット絵を組み立て、
「LIVE 配信開始」のカットインが立ち上がるのが同じフレームに乗っている。
画面遷移に重なるので今は放置しているが、**敵を減らしても消えない**種類の
引っかかりで、詰めるならここが次になる。
...
- `dpr` を 2 に上げても悪化しなかった。ソフトウェアラスタライザでは
  塗る面積が素直に効かないためで、**Retina 実機で同じとは言えない**
- コール & レスポンスを押している状態は測っていない（`call: false` の盤面）
