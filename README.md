...
**現在のステータス: M4（周回）完了 + 登場人物 12 人** —— ステージ 12 本（うちボス 2 本）、
**原作の登場人物 12 人**（+ 隠しキャラ 1 人）、敵 11 種。
配置・ポジション強化（6 段階）・覚醒分岐・セットリスト・フォーメーション・才能ボード・
衣装・進化（Ray）・月華の解放・育成・編成・センター・★難度 1〜10・
プロデューサーランク・楽曲レベル（ソロパート）が動作します。
...
| `src/meta/` | セーブ（マイグレーション付き）・育成・才能ボード・進化・衣装・ランク・楽曲レベル・隠し要素 |
...
- **原作の関係が配置になる**: 犬DOGE はかぐやの隣で速くなり、FUSHI はヤチヨの隣で本領を出すが
  かぐやと犬DOGE の隣では機嫌が悪くなる（原作で敵対的）。人型でない 2 体は専用のドット絵で描く
