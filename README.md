...
> ステージごとの BPM と構成に合わせてその場で組み立てています。
> ただし**その BPM も本作が決めた値**で、原曲のテンポとは関係ありません ——
> 制作側は原曲を聴いていないので、合わせようがないためです。
> 曲名と作曲者・歌唱は原作へのクレジットで、**原作の音源も旋律も使っていません**。
> 原作の曲は Netflix と各配信サービスでお聴きください。
...
- **ドット絵はコード生成**: 画像ファイルを持たず `src/render/sprites.ts` が 48×48 の
  ...
  **髪色も服装も本作が決めたもので、原作のビジュアルは未確認です** ——
  制作環境から公式の絵を見る手段が無いため、似せて描いたものではありません
  （盤面で誰がどこにいるか見分けるための記号として割り当てています）
